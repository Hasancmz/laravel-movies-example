    ...
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
        {{-- @if ()
            <div class="flex-none mx-auto">
                <img src="{{ 'https://image.tmdb.org/t/p/w500/' .  }}" alt="sonic" class="w-80 lg:w-96">  
            </div>            
        @else
            <div class="flex-none mx-auto">
        ...
        <div class="flex-none mx-auto">
            @if ($actor['profile_path'])
                <img src="{{  'https://image.tmdb.org/t/p/w300_and_h450_bestv2/' . $actor['profile_path'] }}" alt="poster" class="">
            @else
            <img src="{{ 'https://ui-avatars.com/api/?size=250&name=' . $actor['name'] }}" alt="poster" class="">
            @endif
              
            <ul class="flex items-center mt-4">
                @if ($socialMedia['facebook_id'])
                    <li class="mr-6">
                        <a href="{{ 'https://facebook.com/' . $socialMedia['facebook_id'] }}" title="Facebook">
                ...
                @if ($socialMedia['instagram_id'])
                    <li class="mr-6">
                        <a href="{{ 'https://instagram.com/' . $socialMedia['instagram_id'] }}" title="Instagram">
                ...
                @if ($socialMedia['twitter_id'])
                    <li class="mr-6">
                        <a href="{{ 'https://twitter.com/' . $socialMedia['twitter_id'] }}" title="Twitter">
        ...
        <div class="mx-auto mt-10 md:ml-24">
            ...
            <div class="flex flex-wrap items-center text-gray-400 text-sm" >
                @if ($age)
                    ...
                    <span class="ml-2">{{ $date }} ({{ $age }} years old ) {{ $actor['place_of_birth'] }}</span>
                @endif

            </div>

            <p class="text-gray-300 mt-8 break-all">{{ $actor['biography'] }}</p>
...
<div class="credits border-b border-gray-800">
    <div class="container mx-auto px-4 py-16">
        ...
        <ul class="list-disc leading-loose pl-5 mt-8">
            @foreach ($starringMovies as $starringMovie)   
                @if ($starringMovie['media_type'] === 'movie')
                    <li>
                        @if (isset($starringMovie['release_date']))
                            <span>{{ Carbon\Carbon::parse($starringMovie['release_date'])->format('Y') }}</span>    
                        @else
                            <span class="text-lg">xxxx</span>
                        @endif
                        <a href="{{ route('movies.show', $starringMovie['id']) }}" class="text-l hover:underline text-yellow-700 ml-2 font-semibold">{{ $starringMovie['title'] }}</a>
                        <span class="mx-2">|</span>  
                        <span class="font-light">{{ $starringMovie['character'] }}</span>
                    </li>    
                @elseif($starringMovie['media_type'] === 'tv')
                    <li>
                        @if (isset($starringMovie['first_air_date']))
                            <span>{{ Carbon\Carbon::parse($starringMovie['first_air_date'])->format('Y') }}</span>    
                        @else
                            <span class="text-lg">xxxx</span>
                        @endif
                        <a href="{{ route('movies.show', $starringMovie['id']) }}" class="text-l hover:underline text-yellow-700 ml-2 font-semibold">{{ $starringMovie['name'] }}</a>
                        <span class="mx-2">|</span>       
                        <span class="font-light">{{ $starringMovie['character'] }}</span>            
                    </li>
                @endif           
